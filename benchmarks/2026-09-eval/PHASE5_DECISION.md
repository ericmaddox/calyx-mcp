...
**Evaluator Run:** `scripts/eval_generalization.py` (Commit `d357f2e`, `main @ 505f358`)  
...
| Metric | Pre-Registered Target | Measured (Standard Hasher) | Measured (Abstracted Tokenizer) | Verdict |
| :--- | :---: | :---: | :---: | :---: |
...
| **Latency (p50)** | $< 5.0\text{ ms}$ | **2.01 ms** (real-path) / **0.22 ms** (in-memory) | **2.15 ms** | **PASS** |

---

## 2. Root Cause Analysis

Fly-LSH projects code features into a 2,048-dimensional Kenyon cell space with top ~5% Winner-Take-All sparsity ($k=102$). The `avoid` threshold requires Jaccard similarity $\ge 0.65$ (at least 81 of 102 Kenyon cells must match).

1. **Identifier Renaming Collapse (`rename` = 0.0% recall):**
   In the standard tokenizer, identifiers (`sym:{name}`) carry weight 1.5 and dominate dense CRC32 bucketing and bigrams (`bi:{w1}_{w2}`). Renaming local variables changes almost the entire token distribution, collapsing Kenyon cell overlap to near zero.
...
4. **Tokenizer Experiment Findings:**
   Replacing local variable identifiers with positional role placeholders (`var_0`, `var_1`) increased recall from 4.44% to 34.44% without any increase in unrelated false positives. However, 34.44% remains well below the 80.0% target necessary to claim robust generalization across arbitrary novel variants.
...
3. **Disclosure of Empirical Limits:** Rather than claiming "100% bug similarity" across novel paraphrased variants, documentation must transparently publish the pre-registered benchmark results and clarify that broad semantic generalization across restructured logic requires full LLM semantic evaluation.
